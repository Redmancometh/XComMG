...
public class AskConfirmReadyRequest extends MGRequest
{
	public AskConfirmReadyRequest(String channel)
	{
		super(channel);
	}
	...
	public void executeRequest(String server, UUID uuid, GameType type)
	{
		CompletableFuture.runAsync(() ->
		{
			...
			try
			{
				request = new MessageRequest(server, super.getChannel(), uuid.toString() + ":" + type.getName());
